...
public class AlignToCoralStation extends Command {
  ...
  public AlignToCoralStation(Drivetrain drivetrain, Limelight limelight, double RotationSpeed) {
    this.limelight = limelight;
    this.drivetrain = drivetrain;
    this.RotationSpeed = RotationSpeed;
    this.teleopDrive = (TeleopDrive) drivetrain.getDefaultCommand();
    rotationPID.enableContinuousInput(-180, 180);
    Rotation2d targetAngle = Rotation2d.fromDegrees(drivetrain.getHeading()).plus(Rotation2d.fromRadians(limelight.getRotateAngleRadMT2("limelight-coral")));
  ...
  public void execute() {
    ...
    Rotation2d targetAngle = Rotation2d.fromDegrees(drivetrain.getHeading()).plus(Rotation2d.fromRadians(limelight.getRotateAngleRadMT2(CORAL_LL)));
    rotationPID.setSetpoint(MathUtil.inputModulus(targetAngle.getDegrees(), -180, 180));
    double rotationDrive = rotationPID.calculate(drivetrain.getHeading());
    if (!limelight.seesTag(CORAL_LL)) rotationDrive = RotationSpeed;
